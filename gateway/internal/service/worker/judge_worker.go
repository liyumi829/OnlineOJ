...
func (w *JudgeWorker) ExecuteTask(ctx context.Context, task *entity.JudgeTask) error {
	...
	// 6. 整理结果
	return w.finalizeSuccess(ctx, task, rpcResp)
}
...
// finalizeSuccess 解析 Judge RPC 响应并写回数据库
func (w *JudgeWorker) finalizeSuccess(ctx context.Context, task *entity.JudgeTask, resp *pb.JudgeResponse) error {
	if resp == nil {
		return w.taskService.FinalizeJudgeFailure(ctx, task.SubmissionID, task.TaskID, "judge rpc response is nil", false)
	}
	...
	return w.taskService.FinalizeJudgeSuccess(
		ctx,
		task.SubmissionID,
		task.TaskID,
